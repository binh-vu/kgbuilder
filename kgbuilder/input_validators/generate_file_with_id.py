...
def add_id_to_mineral_site(json_data, new_json_folder, file_name_without_path):
    ...
    file_to_write = Path(new_json_folder) / file_name_without_path
...
def generate_file_with_id_main(
    filename: Path | str, new_json_folder: Path | str, verbose: bool = False
):
    ...
    if validator_utils.is_json_file_under_data(file_path):
        ...
        if "MineralSite" in json_data:
            json_data = add_id_to_mineral_site(
                json_data, new_json_folder, file_name_without_path
            )
        elif "MineralSystem" in json_data:
            json_data = add_id_to_mineral_system(
                json_data, new_json_folder, file_name_without_path
            )
...
if __name__ == "__main__":
    ...
    generate_file_with_id_main(filename, new_json_folder, verbose=True)
